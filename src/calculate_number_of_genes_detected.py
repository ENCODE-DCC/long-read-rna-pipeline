...
def main(args):
    ...
    gene_counts = calculate_abundances_aggregated_by_gene(abundance_filtered, args.counts_colname)
    number_of_genes_detected = sum(gene_counts >= 1)
    number_of_genes_record = QCMetricRecord()
    number_of_genes_metric = QCMetric('number_of_genes_detected', {'number_of_genes_detected': number_of_genes_detected})
